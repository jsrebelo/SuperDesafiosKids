...
export class MathExerciseGenerator {
  ...
  public generate(skillId: SkillId, level: number): MathExercise {
    if (skillId === "math.counting") return this.counting(level);
    if (skillId === "math.addition") return this.addition(level);
  ...
  private build(
    ...
    while (options.size < 4 && guard < 50) {
      ...
      const candidate = Math.max(
        min,
        Math.min(max, answer + this.integer(-3, 3)),
      );
      options.add(candidate);
    }

    for (let value = min; options.size < 4 && value <= max + 3; value += 1) {
      options.add(value);
    }
